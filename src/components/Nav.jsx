          ...
          <div className="hidden md:flex items-center gap-1">
            {navLinks.map((link) => (
              ...
              >
                {link.label}
                {isActive(link.path) && (
                  <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-1/2 h-0.5 bg-primary rounded-full animate-in fade-in slide-in-from-bottom-1 duration-300" />
                )}
              </Link>
            ))}
          </div>
          ...
          <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
            ...
            <SheetContent side="right" className="w-70 sm:w-[320px] p-0">
              <div className="flex flex-col h-full">
                ...
                {/* Navigation Links - Clean List Style */}
